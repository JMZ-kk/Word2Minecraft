...
class OpenAIGenerator(Generator):
    ...
    def create_story(self, story_paragraphs, total_objectives):
        ...
        story_prompt = (f"Write a {story_paragraphs[0]}-{story_paragraphs[1]} paragraph story "
                        f"which has characters including the protagonist trying to achieve something "
                        f"and the antagonist wanting to stop the protagonist. The story should describe "
                        f"an environment(s) where the story is set up. There should be {total_objectives} "
                        f"objectives for the protagonist in the story. One of them should be to defeat "
                        f"the antagonist somehow. Other objectives can be: "
                        f"finding the exit of a complex labyrinth, "
                        f"finding a chest in the map, "
                        f"surviving waves of enemies, or "
                        f"gathering some items in the map and sending to somebody. "
                        f"You can add some NPCs in the story")
    ...
    def map_tiles_to_chars(self, story, story_prompt, character_discriptions, character_prompt, tileset_discriptions,
                           tileset_prompt):
    ...
    def extract_goals(self, story, story_prompt, character_discriptions, character_prompt, tileset_discriptions,
                      tileset_prompt, tileset_map_discriptions, tileset_map_prompt):
    ...
    def extract_important_tiles(self, story, story_prompt, character_discriptions, character_prompt,
                                tileset_discriptions, tileset_prompt, tile_map_dict, tileset_map_discriptions,
                                tileset_map_prompt, goal_discriptions, goal_prompt):
    ...
    def extract_walkable_tiles(self, story, story_prompt, character_discriptions, character_prompt,
                               tileset_discriptions, tileset_prompt, tile_map_dict, tileset_map_discriptions,
                               tileset_map_prompt, goal_discriptions, goal_prompt):
    ...
    def post_processing_building_tiles(self,tiles_1st_layer):
        pick_building_prompt = f"Considering the following tileset\n{tiles_1st_layer}\n create a Python list of at most 3 building tiles in the 2D tilemap world. Only return the building tiles."
        building_tile_discriptions = openai.ChatCompletion.create(model=self.model, messages=[
            {"role": "user", "content": pick_building_prompt}
        ], temperature=1)

        self.total_input_tokens.append(building_tile_discriptions["usage"]["prompt_tokens"])
        self.total_output_tokens.append(building_tile_discriptions["usage"]["completion_tokens"])
        building_tiles_list = extract_list(building_tile_discriptions['choices'][0]['message']['content'])
        print(building_tiles_list)
        print("\n")
        return building_tiles_list, building_tile_discriptions

    def extract_building_tiles(self, story, story_prompt, character_discriptions, character_prompt,
                               tileset_discriptions, tileset_prompt, tile_map_dict, tileset_map_discriptions,
                               tileset_map_prompt, goal_discriptions, goal_prompt,object_tiles_list):
        print("Extracting building tiles..")
        possible={}
        for key in tile_map_dict:
            val=tile_map_dict[key]
            if f'''{val}''' not in object_tiles_list:
                possible[key]=val
        building_tile_prompt = f"Considering the above goals that you extracted from the story and the following tileset\n{possible}\n create a Python list of at most 3 building tiles in the 2D tilemap world. Only return the building tiles."
        building_tile_discriptions = openai.ChatCompletion.create(model=self.model, messages=[
            {"role": "user", "content": story_prompt},
            {"role": "assistant", "content": story['choices'][0]['message']['content']},
            {"role": "user", "content": character_prompt},
            ...
            {"role": "user", "content": goal_prompt},
            {"role": "assistant", "content": goal_discriptions['choices'][0]['message']['content']},
            {"role": "user", "content": building_tile_prompt}
        ],temperature=1)

        self.total_input_tokens.append(building_tile_discriptions["usage"]["prompt_tokens"])
        self.total_output_tokens.append(building_tile_discriptions["usage"]["completion_tokens"])
        print("\n")
        print(building_tile_discriptions['choices'][0]['message']['content'])
        building_tiles_list = extract_list(building_tile_discriptions['choices'][0]['message']['content'])
        print(building_tiles_list)
        print("\n")
        return building_tiles_list, building_tile_discriptions, building_tile_prompt

    def extract_interactive_object_tiles(self, story, story_prompt, character_discriptions, character_prompt,
                                         tileset_discriptions, tileset_prompt, tile_map_dict, tileset_map_discriptions,
                                         tileset_map_prompt, goal_discriptions, goal_prompt):
        print("Extracting interactive object tiles..")
    ...
    def world_generation(self, rounds, previous_story, story_paragraphs, total_objectives, previous_tile_map,
                         previous_map, previous_eval, story, story_prompt, character_discriptions,
                         character_discriptions_dict, character_prompt, tileset_discriptions, tileset_prompt,
                         tile_map_dict, tileset_map_discriptions, tileset_map_prompt, goal_discriptions, goal_prompt,
                         important_tiles_list, important_tile_discriptions, important_tile_prompt, walkable_tiles_list,
                         walkable_tile_discriptions, walkable_tile_prompt, building_tiles_list,
                         building_tile_discriptions, building_tile_prompt, save_dir):
        ...
        if rounds == 0 or history_to_keep > 0:
            ...
            world_prompt = f"Using the following tile to character mapping:\n{tile_map_dict}\nCreate an entire world on a tile-based grid. Do not create things that would need more than one tile. Also, following characters are important to place:\n{important_tiles_list}\n, walkable tiles:\n{walkable_tiles_list}\n and building tiles:\n{building_tiles_list}\n. Use {no_of_important_tiles} important tiles to create the world. The map should contain at least 20 rows and 20 columns. Do not place the protagonist, the antagonist and the interactive objects of the story right now. Only create the world right now. Create it is a string format with three backticks to start and end with (```) and not in a list format."
        else:
        ...
        while not done:
            try:
                ...
                world_map_fixed = remove_extra_special_chars(world_map_raw)# remove additional character
                ...
                world_with_characters_prompt = (f"Now that you have created the following world map:\n{world_map_fixed}\n "
                                                f"Place only the protagonist, the antagonist and the interactive objects of the story. "
                                                f"Do not change anything in the world, just place only the protagonist, "
                                                f"the antagonist and the interactive objects in the world."
                                                f"The antagonist and protagonist should not share the same row or column.")
                ...
                world_map_raw_with_chars = extract_between_ticks(
                    world_with_characters_discriptions['choices'][0]['message']['content'])
                ...
                world_eval_dict, self.total_input_tokens, self.total_output_tokens = evaluator.evaluate_world(
                    map=world_map_fixed_with_chars,
                ...
                llm_agent_reward, astar_path, objectives = self.action_generation(rounds,
                                                                                  story['choices'][0]['message'][
                                                                                      'content'], "protagonist",
                                                                                  "antagonist",
                                                                                  character_discriptions_dict,
                                                                                  world_map_fixed,
                                                                                  world_map_fixed_with_chars,
                                                                                  used_char_dict, tile_map_dict,
                                                                                  "color_tiles_img_with_char",
                                                                                  "char_color_map",
                                                                                  walkable_tile_discriptions['choices'][
                                                                                      0]['message']['content'],
                                                                                  important_tile_discriptions[
                                                                                      'choices'][0]['message'][
                                                                                      'content'],
                                                                                  goal_discriptions['choices'][0][
                                                                                      'message']['content'], save_dir)

                world_eval_dict["agent_reward"] = llm_agent_reward
                world_eval_dict["astar_path"] = astar_path

                story_paragraphs, total_objectives, no_of_important_tiles, bad_feedback_prompt, good_feedback_prompt = self.feedback_checks(
                    rounds, world_eval_dict, previous_eval, story_paragraphs, total_objectives, no_of_important_tiles)
    ...
    def action_generation(self, round,
        ...
        try:
            while not except_done:
                ...
                objective_tile_system = ("You are a great planner in 2D game. You plan objectives "
                                         "for the protagonist of the game. All objectives should "
                                         "match the goals extracted from the story. One of them should be to defeat "
                                         "the antagonist somehow. Other objectives can be: "
                                         "finding the exit of a complex labyrinth, "
                                         "finding a chest in the map, "
                                         "surviving waves of enemies, or "
                                         "gathering some items in the map."
                                         "Return a Python dictionary "
                                         "of the objective as the key and a tile that achieves the "
                                         "objective and the position of the tile. For example "
                                         "'Objective': ['A', 6, 1]. Only return a Python dictionary. "
                                         "Do not return a python response.")
                objective_tile_prompt = (f"Given the story:\n{story}\n a 2D tile map of a world was created"
                                         f" for the story:\n{world_map_fixed_with_chars}\n The tile map "
                                         f"was created using the following tile to character mapping:"
                                         f"\n{tileset_used_dict}\n You are also provided with the description "
                                         f"of the goals:\n{goal_discriptions}\n and walkable tiles:"
                                         f"\n{walkable_tiles_list}\n and interactive object tiles:\{object_tiles_list}\n "
                                         f"Taking this information into your context, create the objectives "
                                         f"to achieve and also provide the tile that you will pick up, reach "
                                         f"the position at or hit the enemy. Return a Python dictionary of the "
                                         f"objective as the key and a tile that achieves the objective and the "
                                         f"position of the tile. The pattern should be 'Objective': ['tile', row, column]"
                                         f", for example 'Objective': ['A', 6, 1], thus the first element would "
                                         f"be the tile and second and third elements of the list will be position "
                                         f"of the tile. Return strictly in this format.")
                ...
                for _keys, str_obj in objective_tile_dict.items():
                    ...
                    objective_tile_list.append((temp_list[1]-1, temp_list[2]-1))
                ...
                while not solving:
                    try:
                        ...
                        game_state = WorldState(walkable_tiles_list, object_tiles_list, parsed_world_map,
                                                objective_tile_list)
                        game_state = game_state.stringInitialize(parsed_world_map, objective_tile_list)
                        astar_agent = EnhancedAStarWorldAgent(walkable_tiles_list, objective_tile_list, game_state,
                                                              object_tiles_list)
                ...
                env = Word2WorldEnv(walkable_tiles_list, None, None, world_map_fixed,
                                    world_map_fixed_with_chars, object_tiles_list, "#")
                agent = LLMAgent()
                state = env.reset()
                ...
                while not done:
                    ...
                    for i in range(len(objective_tile_dict)):
                        ...
                        distance_from_objective = (abs(objective_pos[1] - protagonist_position[0]),
                                                   abs(objective_pos[2] - protagonist_position[1]))
                        print(
                            f"distance from current objective: [{distance_from_objective[0]}, {distance_from_objective[1]}]")
                        print("\n")
                        ...
                        if (distance_from_objective[0] > 5 and distance_from_objective[0] < 8) or (
                                distance_from_objective[1] > 5 and distance_from_objective[1] < 8):
                            reward -= 50
                        ...
                        if (distance_from_objective[0] <= 5 and distance_from_objective[0] > 3) and (
                                distance_from_objective[1] <= 5 and distance_from_objective[1] > 3):
                            reward += 25
                            reward_feedback += f"You were close to the objective tile so you were given a reward of 25 points"
                        if (distance_from_objective[0] < 3 and distance_from_objective[0] > 1) and (
                                distance_from_objective[1] < 3 and distance_from_objective[1] > 1):
                            reward += 50
                            reward_feedback += f"You were very close to the objective tile so you were given a reward of 50 points"

                        if (distance_from_objective[0] <= 1) and (
                                distance_from_objective[1] > 1 and distance_from_objective[1] <= 5):
                            reward += 50
                            reward_feedback += f"You were very close to the objective tile so you were given a reward of 50 points"
                        if (distance_from_objective[1] <= 1) and (
                                distance_from_objective[0] > 1 and distance_from_objective[0] <= 5):
                            reward += 50
                            reward_feedback += f"You were very close to the objective tile so you were given a reward of 50 points"

                        if (distance_from_objective[0] <= 1 and distance_from_objective[1] <= 1) or \
                                check_discriptions['choices'][0]['message']['content'] == "Complete":

                            if (distance_from_objective[0] == 0 and distance_from_objective[1] == 0) and \
                                    check_discriptions['choices'][0]['message']['content'] == "Complete":
                                reward += 200
                                # objective_flag = True
