...
all_done_tag='world_created'+str(random.randint(0,99999))
...
if args.game_path:
    ...
    custom_path='word2world/desert/turbo_42580_1.json'
...
objective_types = ['Defeat the enemy', 'Chat with NPC', 'Navigate the maze to find an exit', 'Survive waves of enemies',
                   'Collect Items']
# order_prompt, order_discriptions, order_dict = extract_goal_order(goals,objective_list)
...
obj_tag_list=[]
for obj in objective_list:
    if obj.goal!='Defeat the enemy':
        obj_tag_list.append('tag_'+str(obj.id))
...
if len(clist) > 0:
    ...
    for key in item_dict.keys():
        for obj in objective_list:
            if obj.description == key:
                if obj.item in block_data:
                    obj.item = item_dict[key]
                else:
                    obj.item = random.choice(block_data)
                print(obj.item)
occupied_positions = []
                ...
                for m in [i - block_size + 1, i]:
                    for n in [j - block_size + 1, j]:
                        try:
                            ...
                            print(e)
                if biggest_m > -1:
                    update_map(check, biggest_m, biggest_n, block_size, tar, grid_world,check4show)
print(grid_world)
#------------------------------------------scaling problem--------------------------------------
...
for x in range(len(base_map)):
    for z in range(len(base_map[x])):
        if (x, z) in occupied_positions:
            for obj in objective_list:
                obj_fin = 'tag_' + str(obj.id)
                if obj.goal == 'Defeat the enemy':
                    continue
                try:
                    if obj.coordinates == (x, z):
                        start_tag = 'task_' + str(obj.id)
                        ...
                        if obj.goal == 'Chat with NPC':
                            ...
                            command = (
                                ...
                                base_z + z) + ' {'
                                        ...
                                        0] + '\\\",\\\"color\\\":\\\"red\\\"}\',CustomNameVisible:1b'
                            ...
                            command2 = (
                                    'execute as @a[tag=' + start_tag + ',tag=!' + obj_fin + '] run tellraw @s {\\\"text\\\":\\\"' + obj.description + '\\\",\\\"color\\\":\\\"green\\\"}'
                            )

                            command3 = (
                                    'execute as @a[tag=' + start_tag + ',tag=!' + obj_fin + '] run tag @s add ' + obj_fin)
    ...
    #-----------------------------------place sub map------------------------------
                        ...
                        for m in range(len(obj.map)):
                            for hh in range(3):
                                editor.placeBlock((sum_x_len + m, base_y+hh, base_z - 1),
                                                  Block('minecraft:barrier'))
                                editor.placeBlock((sum_x_len + m, base_y+hh, base_z + len(obj.map[0])),
                                                  Block('minecraft:barrier'))
                        for n in range(len(obj.map[0])):
                            for hh in range(3):
                                editor.placeBlock((sum_x_len - 1, base_y+hh, base_z + n),
                                                  Block('minecraft:barrier'))
                                editor.placeBlock((sum_x_len + len(obj.map), base_y+hh, base_z + n),
                                                  Block('minecraft:barrier'))
                        for m in range(len(obj.map)):
                            for n in range(len(obj.map[0])):
                                tx = sum_x_len + m
                                ...
                                if obj.goal == 'Navigate the maze to find an exit':
                                    ...
                                    if obj.map[m][n] == 0:
                                        place_block(editor,tx, ty, tz, path_tile,block_data)
                                    else:
                                        place_block(editor,tx, ty, tz, wall_tile,block_data)
                                    ...
                                    try:
                                        place_block(editor,tx, ty, tz, obj.tile_dict[obj.map[m][n]],block_data)
                                    except:
                                        place_block(editor,tx, ty, tz, tileset.get(default_walkable_tile),block_data)
    ...
    # -----------------------------------place sub map done------------------------------
                        if obj.goal == 'Survive waves of enemies':
                            timer = 't' + str(obj.id)
                            timer_b = 'tb' + str(obj.id)
                            tag = 'rs' + str(obj.id)
                            tag_b = 'rsb' + str(obj.id)
                            ...
                            editor.placeBlock((maze_end_x, base_y - 2, maze_end_z - 1), Block('minecraft:command_block',
                                        data='{"Command": "scoreboard objectives add ' + timer_b + ' dummy","auto": 1})'))
                            editor.placeBlock((base_x + x, base_y - 1, base_z + z),
                                              Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                    data='{"Command": "scoreboard players set @a ' + timer + ' 10","auto": 1})'))
                            editor.placeBlock((base_x + x, base_y - 2, base_z + z), Block('minecraft:chain_command_block', states={'facing': 'down'},
                                        data='{"Command": "/tag @a add '+tag+'","auto": 1})'))

                            countdown(editor,timer, tag, 'Enemy is coming!', maze_end_x, base_y - 3, maze_end_z - 1)
                            for en in range(obj.enemy_num):
                                ex=random.randint(maze_end_x-len(obj.map)+2,maze_end_x-2)
                                ez=random.randint(maze_end_z-len(obj.map[0])+2,maze_end_z-2)
                                time_command_5 = 'execute as @a[scores={' + timer + '=0},tag=' + tag + '] run summon ' + obj.enemy_name + ' ' + str(
                                    ex) + ' ' + str(base_y + 2) + ' ' + str(ez)
                                editor.placeBlock((maze_end_x, base_y - 6-en, maze_end_z-1),
                                              Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                    data=f'{{"Command": "{time_command_5}","auto": 1}}'))
                            editor.placeBlock((maze_end_x, base_y - 6-obj.enemy_num, maze_end_z-1),
                                              Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                    data='{"Command": "execute as @a[scores={' + timer + '=0},tag=' + tag + ']'
                                                                ' run tag @s add ' + tag_b+'","auto": 1})'))
                            editor.placeBlock((maze_end_x, base_y - 7 - obj.enemy_num, maze_end_z - 1),
                                              Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                    data='{"Command": "execute as @a[scores={' + timer + '=0},tag=' + tag + '] '
                                                'run scoreboard players set @a ' + timer_b + ' 20","auto": 1})'))
                            editor.placeBlock((maze_end_x, base_y - 8 - obj.enemy_num, maze_end_z - 1),
                                              Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                    data='{"Command": "execute as @a[scores={' + timer + '=0},tag=' + tag + '] '
                                                'run give @s minecraft:shield","auto": 1})'))
                            editor.placeBlock((maze_end_x, base_y - 9 - obj.enemy_num, maze_end_z - 1),
                                              Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                    data='{"Command": "execute as @a[scores={' + timer + '=0},tag=' + tag + '] '
                                                'run give @s minecraft:diamond_sword","auto": 1})'))
                            countdown(editor,timer_b, tag_b, 'Survive!', maze_end_x-2, base_y - 3, maze_end_z - 1)

                            editor.placeBlock((maze_end_x - 2, base_y - 6, maze_end_z - 1),
                                              Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                    data='{"Command": "execute as @a[scores={' + timer_b + '=0}, tag=' + tag_b + ',tag=!' + obj_fin + ']'
                                                    ' run tp @p ' + str(
                                                        base_x + player_pos[1]) + ' ' + str(
                                                        base_y + 4) + ' ' + str(
                                                        base_z + player_pos[0]) + '","auto": 1}'))
                            editor.placeBlock((maze_end_x-2, base_y - 7, maze_end_z - 1),
                                              Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                    data='{"Command": "execute as @a[scores={' + timer_b + '=0}, tag=' + tag_b + ',tag=!' + obj_fin + ']'
                                                ' run tag @s add ' + obj_fin + '","auto": 1}'))

                            time_command_6='execute as @a[scores={'+timer+'=..0}] run scoreboard players reset @a '+timer
                            editor.placeBlock((maze_end_x - 1, base_y - 2, maze_end_z),
                                              Block('minecraft:repeating_command_block', states={'facing': 'down'},
                                                    data=f'{{"Command": "{time_command_6}","auto": 1}}'))
                            time_command_7 = 'execute as @a[scores={' + timer_b + '=..0}] run scoreboard players reset @a ' + timer_b
                            editor.placeBlock((maze_end_x - 1, base_y - 3, maze_end_z),
                                              Block('minecraft:repeating_command_block', states={'facing': 'down'},
                                                    data=f'{{"Command": "{time_command_7}","auto": 1}}'))

    #-------------------------------tp-----------------------
                        editor.placeBlock((base_x + x, base_y, base_z + z), Block('minecraft:command_block', states={'facing': 'down'},
                                                                               data='{"Command": "execute as @p[tag='+all_done_tag+'] run tp @p ' + str(
                                                                                   sum_x_len + 1) + ' ' + str(
                                                                                   base_y + 4) + ' ' + str(
                                                                                   base_z + 1) + '"}'))
    ...
    #-------------------------------tp back-----------------------
                        ...
                        if obj.goal == 'Navigate the maze to find an exit':
                            ...
                            command4 = f'tag @p add tag_{obj.id}'
                            ...
                            editor.placeBlock((maze_end_x, base_y - 2, maze_end_z),
                                                  Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                        data=f'{{"Command": "{command3}","auto": 1}}'))
                            editor.placeBlock((maze_end_x, base_y - 3, maze_end_z),
                                              Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                    data=f'{{"Command": "{command4}","auto": 1}}'))
                        elif obj.goal == 'Survive waves of enemies':
                            print(f'sur:{obj.id}')
                            text_color = 'yellow'
                        elif obj.goal == 'Collect Items':
                            print(f'col:{obj.id}')
                            cx = random.randint(maze_end_x - len(obj.map) + 1, maze_end_x - 1)
                            cz = random.randint(maze_end_z - len(obj.map[0]) + 1, maze_end_z - 1)
                            chest = Block("chest", data='{Items: [{Slot: 13b, id: ' + obj.item + ', Count: 1b}]}')
                            editor.placeBlock((cx, base_y + 1, cz), chest)
                            command1 = (
                                    'execute as @a[tag=!' + start_tag + '] at @s if entity @a[nbt={Inventory:[{id:\\\"minecraft:' + str(
                                obj.item) + '\\\"}]}] run tag @s add ' + start_tag
                            )
                            command2 = (
                                    'execute as @a[tag=' + start_tag + ',tag=!' + obj_fin + '] at @s if entity @a[nbt={Inventory:[{id:\\\"minecraft:' + str(
                                obj.item) + '\\\"}]}] run tag @s add ' + obj_fin
                            )
                            editor.placeBlock((maze_end_x, base_y-1, maze_end_z),
                                              Block('minecraft:repeating_command_block', states={'facing': 'down'},
                                                    data=f'{{"Command": "{command1}","auto": 1}}'))
                            editor.placeBlock((maze_end_x, base_y - 2, maze_end_z),
                                              Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                    data='{"Command": "execute as @a[tag=' + start_tag + ',tag=!' + obj_fin + '] '
                                                                      'run tp @p ' + str(
                                                        base_x + player_pos[1]) + ' ' + str(
                                                        base_y + 4) + ' ' + str(
                                                        base_z + player_pos[0]) + '","auto": 1}'))
                            ...
                            text_color = 'green'
                        title_command = (
                                    'summon armor_stand ' + str(base_x + x) + ' ' + str(base_y + 1) + ' ' + str(base_z + z) + ' {'
...
time_command_1='execute if score global Tick matches 22 run scoreboard players set global Tick 0'
...
command5 = f'tag @p add {all_done_tag}'
editor.placeBlock((base_x + x, base_y-4, base_z + z),
                    Block('minecraft:command_block', states={'facing': 'down'},
        data=f'{{"Command": "{command5}","auto": 1}}'))
