...
def extract_npc_appearance(goal_prompt, goal_discriptions,npc_prompt,npc_discriptions,name_list):
    cfg = Config()
    npc_appearance_prompt = (f'For each npc in \n{name_list}\n, choose one of [golden,leather,diamond,iron] '
                             f'to describe his or her armor.'
                   f'return a python dict where the key is the name of the npc and the value is his or her armor.')
...
def extract_tool(goal_prompt, goal_discriptions,npc_prompt,npc_discriptions,tool_list,name_list):
    cfg = Config()
    tool_prompt = (f'You will be given a list of tools: \n{tool_list}\n and a list of npc names: \n{name_list}\n. '
                   f'Find one of them as hand item for each npc.'
                   f'return a python dict where the key is the name of the npc and the value is his or her hand item.')
...
npc_dict,npc_prompt,npc_discriptions=extract_npc(goal_prompt, goal_discriptions, goal_dict_2['Interact with NPC'])
...
name_list=list(npc_dict_reverse.keys())
npc_tool=extract_tool(goal_prompt, goal_discriptions,npc_prompt,npc_discriptions, tool_list,name_list)
npc_appearance_dict=extract_npc_appearance(goal_prompt, goal_discriptions,npc_prompt,npc_discriptions,name_list)
...
tool_block=extract_enemy(enemy,tool_list)
    ...
    for z in range(len(base_map[x])):
        ...
        for npc_name, value in npc_positions.items():
            try:
                ...
                hand_tool=npc_tool[npc_name]
                cloth=npc_appearance_dict[npc_name]
            except:
                print(npc_positions)
                print(npc_dict_reverse)
                print(npc_tool)
                print(npc_appearance_dict)
            if hand_tool=='empty':
                hand_tool_str=''
            else:
                hand_tool_str='HandItems:[{id:\\\"minecraft:'+hand_tool+'\\\",Count:1},{}],'
                ...
                try:
                    command = (
                        'summon armor_stand '+str(base_x+x)+' '+str(base_y+1)+' '+str(base_z+z)+' {'
                            'CustomName:\'{\\\"text\\\":\\\"'+npc_name_str+'\\\"}\',CustomNameVisible:1b'
                        ',ArmorItems:[{id:\\\"minecraft:'+cloth+'_boots\\\",Count:1},{id:\\\"minecraft:'+cloth+'_leggings\\\",'
                            'Count:1},{id:\\\"minecraft:'+cloth+'_chestplate\\\",Count:1},'
                        '{id:\\\"minecraft:'+cloth+'_helmet\\\",Count:1}],'+hand_tool_str+'}'
                    )
                except Exception as e:
                    command = (
                        ...
                        base_z + z) + ' {'
                                      'CustomName:\'{\\\"text\\\":\\\"' + npc_name_str + '\\\"}\',CustomNameVisible:1b'
                                                                                         ',ArmorItems:[{id:\\\"minecraft:golden_boots\\\",Count:1},{id:\\\"minecraft:golden_leggings\\\",'
                                                                                         'Count:1},{id:\\\"minecraft:golden_chestplate\\\",Count:1},'
        ...
        for tuple in trap_positions_list:
            if tuple[0] == x and tuple[1] == z:
                ...
                print(f'trap at{base_x+x},{base_z+z}')
                command1 = (
                        f'execute as @a[x={base_x+x},y={base_y+1},z={base_z+z},distance=..3] run effect give @s nausea 10 1'
                )
                editor.placeBlock((base_x + x, base_y-1, base_z + z),
                                  Block('minecraft:repeating_command_block', states={'facing': 'down'},
                                        data=f'{{"Command": "{command1}","auto": 1}}'))
