        ...
        try:
            ...
            for i in range(len(grid_world)):
                for j in range(len(grid_world[0])):
                    if (i, j) in occupied_positions or grid_world[i][j] in ['#', '@']:
                        check[i][j] = 3
                        unwalkable_map[i][j] = 2
                            ...
                            if biggest_m > -1:
                                for i in range(block_size):
                                    for j in range(block_size):
                                        unwalkable_map[biggest_m + i][biggest_n + j] = 1
            ...
            res=[]
            for iii in range(len(grid_world)):
                res.append([])
                for jjj in range(len(grid_world[0])):
                    res[iii].append(unwalkable_map[iii][jjj])
            new_file['uw']=res
            with open(f"{dir}/u_{filename}", 'w') as f:
