...
from transformers import (DistilBertTokenizer,
                          DistilBertModel, AutoTokenizer, AutoModel, RobertaModel,AlbertModel)
import torch
...
def batch_similarity(descs1, descs2, model_type='bert'):
    """
    ...
    if model_type=='bert':
        ...
        model = RobertaModel.from_pretrained('microsoft/deberta-base')
...
def gpt_batch_similarity(descs1, descs2):
    similarities = []
...
def find_most_similar_block(list,model_type='bert'):
    blocklist = {}
    try:
        from lookup import BLOCKS
    except:
        from word2world.lookup import BLOCKS,PLANTS
    choice=tuple(set(BLOCKS)-set(PLANTS))
    block_data = [item.replace('minecraft:', '') for item in choice]
    for desc in list:
        similarities = batch_similarity([desc] * len(block_data),
                                                block_data,model_type=model_type)
        max_similarity = max(similarities)
        max_index = similarities.index(max_similarity)
        blocklist[desc] = 'minecraft:'+block_data[max_index]
    return blocklist

def find_most_similar_images(dictionary, csv_path, isMC=False,model_type='bert',interactive_object_tiles=None):
    """
    ...
    if isMC:
        ...
        for desc, tile in dictionary.items():
            desc = desc.replace(' Tile', '').lower()
            choices = []
            for idx in range(len(block_data)):
                if desc in block_data[idx] or block_data[idx] in desc:
                    choices.append(block_data[idx])
            if len(choices) == 0:
                choices=block_data[:]
            if 'tree' in desc:
                choices.append('tree')
            choices.append('wooden house')
            similarities = batch_similarity([desc] * len(choices),
                                            choices, model_type=model_type)
            max_similarity = max(similarities)
            max_index = similarities.index(max_similarity)
            selected_image = choices[max_index]
            images[tile] = selected_image
        ...
        for desc, tile in dictionary.items():
            if not tile.isalpha():
                ...
                similarities = batch_similarity([desc] * len(list(data_char['description'])),
                                                list(data_char['description']))
                max_similarity = max(similarities)
                max_index = similarities.index(max_similarity)
                ...
                # Compare each description with the descriptions in the CSV
                similarities = batch_similarity([desc] * len(list(data['description'])), list(data['description']))
                max_similarity = max(similarities)
                max_index = similarities.index(max_similarity)
            ...
            try:
                while not done:
                    similarity_discriptions = openai_function(model="gpt-4-0613", messages=[
                        {"role": "system",
                         "content": "You are a great sementic similarity checker. You can check for semantic similarities and return without hillucinating. Do not return None. There will always be a word similar to presented description"},
                        {"role": "user",
                         "content": f"out of the following list of description:\n{list(df_char.description)}\nwhich one description matches '{desc}'. Strictly return only the one word that matches the most. Only and only the word that matches. Do not return None. There will always be a word similar to presented description"}
                    ],
                                                              temperature=0.6)
                    ...
                    if str(similarity_discriptions['choices'][0]['message']['content']) == 'None':
                        ...
                        continue
                    selected_image = df_char.loc[df_char['description'] == str(
                        similarity_discriptions['choices'][0]['message']['content']), 'filename'].values[0]
            ...
            try:
                while not done:
                    ...
                    if str(similarity_discriptions['choices'][0]['message']['content']) == 'None':
                        ...
                        continue
                    selected_image = df.loc[df['description'] == str(
                        similarity_discriptions['choices'][0]['message']['content']), 'filename'].values[0]
                    input_cost.append(similarity_discriptions["usage"]["prompt_tokens"])
                    output_cost.append(similarity_discriptions["usage"]["completion_tokens"])
