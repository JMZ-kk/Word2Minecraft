...
class Word2World:
    ...
    def run(self, cfg):
        ...
        import random
        rd=random.randint(0,99999)
        story, story_prompt = generator.create_story(cfg.story_paragraphs, cfg.total_objectives)
        for tt in range(3):
            character_discriptions, character_discriptions_dict, character_prompt, protagonist_name, antagonist_name = generator.extract_character_info(
                story, story_prompt)
            tileset_discriptions, tileset_prompt = generator.extract_tileset_info(story, story_prompt,
                                                                                  character_discriptions, character_prompt)
            tile_map_dict, tileset_map_discriptions, tileset_map_prompt = generator.map_tiles_to_chars(story, story_prompt,
                                                                                                       character_discriptions,
                                                                                                       character_prompt,
                                                                                                       tileset_discriptions,
                                                                                                       tileset_prompt)
            goal_discriptions, goal_prompt = generator.extract_goals(story, story_prompt, character_discriptions,
                                                                     character_prompt,
                                                                     tileset_discriptions, tileset_prompt,
                                                                     tileset_map_discriptions, tileset_map_prompt)
            important_tiles_list, important_tile_discriptions, important_tile_prompt = generator.extract_important_tiles(
                story, story_prompt, character_discriptions, character_prompt,
                tileset_discriptions, tileset_prompt, tile_map_dict,
                tileset_map_discriptions, tileset_map_prompt, goal_discriptions,
                goal_prompt)
            walkable_tiles_list, walkable_tile_discriptions, walkable_tile_prompt = generator.extract_walkable_tiles(story,
                                                                                                                     story_prompt,
                                                                                                                     character_discriptions,
                                                                                                                     character_prompt,
                                                                                                                     tileset_discriptions,
                                                                                                                     ...
                                                                                                                     goal_discriptions,
                                                                                                                     goal_prompt)
            object_tiles_list, object_tile_discriptions, object_tile_prompt = generator.extract_interactive_object_tiles(
                story, story_prompt, character_discriptions, character_prompt,
                tileset_discriptions, tileset_prompt, tile_map_dict,
                tileset_map_discriptions, tileset_map_prompt, goal_discriptions,
                goal_prompt)
            ...
            for rounds in range(cfg.rounds):
                ...
                world_map_fixed, world_map_fixed_with_chars, world_eval_dict, tileset_used_orig, tileset_used_dict, \
                    ...
                    no_of_important_tiles, llm_agent_reward, astar_path = generator.world_generation(rounds,
                                                                                                 self.previous_story,
                ...
                self.worlds_history[f"round_{rounds}"] = {"story": story['choices'][0]['message']['content'],
                                                          "character_information":
                                                              character_discriptions['choices'][0]['message']['content'],
                                                          "tile_mapping": tileset_used_dict,
                                                          "goals": goal_discriptions['choices'][0]['message']['content'],
                                                          "objectives": objectives,
                                                          "important_tiles":
                                                              important_tile_discriptions['choices'][0]['message'][
                                                                  'content'],
                                                          "walkable_tiles":
                                                              walkable_tile_discriptions['choices'][0]['message'][
                                                                  'content'],
                                                          "interactive_object_tiles":
                                                              object_tile_discriptions['choices'][0]['message']['content'],
                                                          "world_1st_layer": {"world": world_map_fixed,
                                                                              "tiles": tileset_used_orig},
                ...
                with open(f"word2world/forest/same_{rd}_{tt}.json", 'w') as f:
                    json.dump(self.worlds_history, f)

                spent_this_gen = (sum(self.total_input_tokens) / 1000) * 0.01 + (
                            sum(self.total_output_tokens) / 1000) * 0.03
