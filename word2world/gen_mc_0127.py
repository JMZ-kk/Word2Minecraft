...
def countdown(timer,tag,prompt_word,x,start_y,z):
...
for x in range(len(base_map)):
            ...
            for obj in objective_list:
                try:
                    if obj.coordinates == (x, z):
                        ...
                        end_tag = 'task_' + str(obj.id) + '_done'
                        ...
                        for m in range(len(obj.map)):
                            for n in range(len(obj.map[0])):
                                ...
                                if obj.goal == 'Navigate the maze to find an exit':
                                    ...
                                    if obj.map[m][n] == 0:
                                        place_block(tx, ty, tz, path_tile)
                                    else:
                                        place_block(tx, ty, tz, wall_tile)
                                    ...
                                    try:
                                        place_block(tx, ty, tz, obj.tile_dict[obj.map[m][n]])
                                    except:
                                        place_block(tx, ty, tz, tileset.get(default_walkable_tile))
    ...
    # -----------------------------------place sub map done------------------------------
                        if obj.goal == 'Survive waves of enemies':
                            ...
                            countdown(timer, tag, 'Enemy is coming!', maze_end_x, base_y - 3, maze_end_z - 1)
                            ...
                            countdown(timer_b, tag_b, 'Survive!', maze_end_x-2, base_y - 3, maze_end_z - 1)

                            editor.placeBlock((maze_end_x-2, base_y - 6, maze_end_z - 1),
                                              Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                    data='{"Command": "execute as @a[scores={' + timer_b + '=0}, tag=' + tag_b + ']'
                                                ' run tag @s add ' + tag_c + '","auto": 1}'))
    ...
    #-------------------------------tp back-----------------------
                        ...
                        if obj.goal == 'Navigate the maze to find an exit':
                            ...
                            text_color = 'yellow'
                            sub_score=obj.enemy_name+'_kills_'+str(obj.id)
                            command_clear = f'setblock {maze_end_x} {base_y} {maze_end_z} minecraft:air'
                            create_board = 'scoreboard objectives add '+sub_score+' minecraft.killed:minecraft.'+obj.enemy_name
                            editor.placeBlock((maze_end_x, base_y-1, maze_end_z),
                                              Block('minecraft:command_block', states={'facing': 'down'},
                                                    data=f'{{"Command": "{create_board}","auto": 1}}'))
                            editor.placeBlock((maze_end_x, base_y-2, maze_end_z),
                                              Block('minecraft:repeating_command_block', states={'facing': 'down'},
                                                    data='{"Command": "execute as @a[scores={'+sub_score+'=3..},tag=!'+end_tag+'] '
                                                         'run tp @p ' + str(
                                                        base_x + player_pos[1]) + ' ' + str(
                                                        base_y + 4) + ' ' + str(
                            ...
                            editor.placeBlock((maze_end_x, base_y - 3, maze_end_z),
                                              Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                    data='{"Command": "execute as @a[scores={'+sub_score+'=3..},tag=!' + end_tag + '] run tag @s add ' + end_tag + '","auto": 1}'))
                            editor.placeBlock((maze_end_x, base_y-4, maze_end_z),
                                              Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                    data='{"Command": "execute as @a[scores={'+sub_score+'=3..}] '
                                                         'run scoreboard players reset @a '+sub_score+'","auto": 1}'))
                            ...
                            command2 = (
                                    'execute as @a[tag=' + start_tag + ',tag=!' + end_tag + '] at @s if entity @a[nbt={Inventory:[{id:\\\"minecraft:' + str(
                                obj.item) + '\\\"}]}] run tag @s add ' + end_tag
                            )
                            editor.placeBlock((maze_end_x, base_y-1, maze_end_z),
                                              Block('minecraft:repeating_command_block', states={'facing': 'down'},
                                                    data=f'{{"Command": "{command1}","auto": 1}}'))
                            editor.placeBlock((maze_end_x, base_y - 2, maze_end_z),
                                              Block('minecraft:chain_command_block', states={'facing': 'down'},
                                                    data='{"Command": "execute as @a[tag=' + start_tag + ',tag=!' + end_tag + '] '
                                                                      'run tp @p ' + str(
                                                        base_x + player_pos[1]) + ' ' + str(
                                                        base_y + 4) + ' ' + str(
                                                        base_z + player_pos[0]) + '","auto": 1}'))
            ...
            if z == player_pos[0] and x == player_pos[1]:
                ...
                try:
                    ...
                    if base_map[x][z] not in walkables and block not in FILTERING:
                        ...
                        place_block(base_x + x, base_y, base_z + z, tile)
